...
def get_model():
    ...
    if not os.path.exists(MODEL_PATH):
        app.logger.error(f"Model file NOT FOUND at {MODEL_PATH}. Using dummy model.")
...
def preprocess_image(file_storage):
    ...
    try:
        ...
        app.logger.error(f"Preprocessing error: {e}")
        raise

def perform_prediction(file_storage):
    """Predict the chess piece."""
    ...
    app.logger.info(f"Result: {class_name} ({confidence:.2%})")
...
def predict():
    app.logger.info("Received POST /predict")
